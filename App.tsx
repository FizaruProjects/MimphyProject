...
import { School, User, Lock, Mail, ArrowRight, Cat, AlertCircle, BrainCircuit } from 'lucide-react';

const MainApp: React.FC = () => {
  const [session, setSession] = useState<UserSession>({ role: null });
  const [authMode, setAuthMode] = useState<'selection' | 'student_login' | 'teacher_login' | 'admin'>('selection');
  ...
  // --- AUTH HANDLERS ---
  const handleLogin = (roleTarget: string) => {
      const res = StorageService.loginUser(formData.email, formData.password);
