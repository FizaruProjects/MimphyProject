...
export const StudentDashboard: React.FC<Props> = ({ session, onLogout }) => {
  ...
  const refreshStudentData = () => {
      ...
      const myResults = allResults.filter(r => r.studentId === session.userId);
      ...
      setAllAchievements(StorageService.getAchievements());
      setAvailablePacketsCount(StorageService.getPackets().length);
  };
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (file && session.userId) {
          ...
          reader.onload = () => {
              ...
              StorageService.updateStudentProfile(session.userId!, { photoUrl: base64 });
  ...
  const updateLearningStyle = (style: LearningStyle) => {
      if (session.userId) {
          StorageService.updateStudentProfile(session.userId, { learningStyle: style });
  ...
  const checkAndStartPacket = () => {
    ...
    const packet = packets.find(p => p.id === code);
  ...
  const finishQuiz = () => {
    ...
    const newResult: StudentResult = {
        ...
        attemptNumber: 0, // Will be calculated in StorageService
        timestamp: Date.now()
    };

    // Save and check achievements
    const unlocked = StorageService.saveResult(newResult);
            ...
            <div className="flex-1 flex items-center justify-center p-4">
                <div className="max-w-md w-full bg-white/80 dark:bg-slate-800/80 backdrop-blur-lg rounded-[2rem] shadow-[0_10px_40px_-10px_rgba(220,38,38,0.3)] dark:shadow-none dark:border dark:border-slate-700 text-center relative overflow-hidden p-8 animate-in zoom-in-95">
                    ...
                    <div className="space-y-4">
                        ...
                        <button onClick={checkAndStartPacket} disabled={!packetCode} className="w-full bg-red-600 text-white py-4 rounded-2xl font-extrabold text-lg hover:bg-red-700 shadow-[0_4px_0_rgb(185,28,28)] active:shadow-none active:translate-y-1 transition-all transform hover:-translate-y-1 disabled:opacity-50 disabled:cursor-not-allowed">Buka Paket 🚀</button>
                    </div>
                </div>
            </div>
            <CopyrightFooter />
            ...
            <div className="max-w-6xl mx-auto p-4 md:p-6 grid grid-cols-1 lg:grid-cols-4 gap-6 flex-1 w-full mb-16 lg:mb-0">
                <div className="lg:col-span-3">
                    ...
                    <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                        ...
                        <button onClick={() => { if (currentQIndex < activePacket.questions.length - 1) setCurrentQIndex(prev => prev + 1); else finishQuiz(); }} className={`w-full md:w-auto flex items-center justify-center px-8 py-3 rounded-xl font-extrabold text-white transition-all shadow-lg transform hover:-translate-y-1 active:translate-y-0 active:scale-95 ${currentQIndex === activePacket.questions.length - 1 ? 'bg-gradient-to-r from-green-400 to-emerald-500 shadow-emerald-200 dark:shadow-emerald-900' : 'bg-gradient-to-r from-red-500 to-orange-600 shadow-red-200 dark:shadow-red-900'}`}>{currentQIndex === activePacket.questions.length - 1 ? 'Selesai!' : 'Lanjut'} <ChevronRight className="ml-2 w-5 h-5" /></button>
                    </div>
                </div>
              ...
              <div className="max-w-2xl mx-auto p-6 w-full flex-1">
                  <div className="bg-white dark:bg-slate-800 rounded-[2rem] p-8 shadow-sm border border-stone-200 dark:border-slate-700">
                      ...
                      <div className="space-y-6">
                          ...
                          <div className="mb-6">
                             <ContributionGraph 
                                data={StorageService.getResults()
                                    .filter(r => r.studentId === session.userId)
                                    .map(r => ({ date: new Date(r.timestamp).toISOString(), count: 1 }))
                                }
                          ...
                          <div className="bg-stone-50 dark:bg-slate-700/30 p-6 rounded-2xl border border-stone-100 dark:border-slate-700">
                              ...
                              <form onSubmit={(e) => {
                                  ...
                                  if (session.userId) {
                                      StorageService.updateStudentProfile(session.userId, updates);
