...
export const TeacherPacketManager: React.FC<Props> = ({ packets, onRefresh }) => {
    ...
    const handleDelete = (packetId: string) => {
        if (StorageService.hasPacketResults(packetId)) {
        ...
        StorageService.deletePacket(packetId);
        onRefresh();
    };
    ...
    const handleSaveEdit = () => {
        ...
        const hasResults = StorageService.hasPacketResults(editData.id);
        if (hasResults) {
        ...
        StorageService.updatePacket(editData);
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left">
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                            ...
                            {packets.map(p => {
                                const hasRes = StorageService.hasPacketResults(p.id);
                                return (
