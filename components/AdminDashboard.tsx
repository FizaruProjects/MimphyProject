...
import { Users, BookOpen, BarChart3, Shield, Key, Power, UserCheck, UserX, Search, AlertCircle, X, AlertTriangle, Lock, LogOut, GraduationCap, School, Plus, BrainCircuit } from 'lucide-react';
...
export const AdminDashboard: React.FC<Props> = ({ onLogout }) => {
    ...
    const refreshData = () => {
        ...
        setPackets(StorageService.getPackets());
    ...
    const handleToggleStatus = (id: string, currentStatus: boolean, type: 'teacher' | 'student') => {
        const action = currentStatus ? 'menonaktifkan' : 'mengaktifkan';
        const role = type === 'teacher' ? 'guru' : 'siswa';
        if(window.confirm(`Yakin ingin ${action} ${role} ini?`)) {
            if (type === 'teacher') StorageService.updateTeacherStatus(id, !currentStatus);
            else StorageService.updateStudentStatus(id, !currentStatus);
            refreshData(); 
        }
    };
    ...
    const executeReset = () => {
        ...
        if (!newPassword.trim()) {
            alert("Mohon masukkan password baru terlebih dahulu.");
            return;
        }

        let success = false;
        if (resetTarget.type === 'teacher') {
            success = StorageService.resetTeacherPassword(resetTarget.id, newPassword);
        } else {
            success = StorageService.resetStudentPassword(resetTarget.id, newPassword);
        }

        if (success) {
            alert(`SUKSES!\nPassword untuk ${resetTarget.name} telah diubah.`);
            refreshData();
        } else {
    ...
    const handleCreateUser = () => {
        ...
        if (showCreateModal === 'teacher') {
            ...
            res = StorageService.registerTeacher(createFormData.name, createFormData.email, createFormData.password, createFormData.subject);
        } else {
             if (!createFormData.className || !createFormData.schoolName) {
                alert("Mohon lengkapi data sekolah dan kelas untuk siswa.");
                return;
            }
            res = StorageService.registerStudent(
                createFormData.name, 
            ...
            {resetTarget && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-red-900/20 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white dark:bg-slate-800 rounded-[2rem] shadow-2xl w-full max-w-md p-8 animate-in zoom-in-95 duration-200 border border-white dark:border-slate-700">
                        ...
                        <div className="mb-6">
                            <p className="text-slate-600 dark:text-slate-300 mb-2 font-medium">Atur password baru untuk {resetTarget.type === 'teacher' ? 'Guru' : 'Siswa'}:</p>
                            <div className="bg-orange-50 dark:bg-orange-900/20 border border-orange-100 dark:border-orange-900/40 p-4 rounded-xl font-bold text-slate-800 dark:text-orange-100 text-center mb-4">
                                {resetTarget.name}
                            </div>
                            ...
                            <div className="relative">
                                ...
                                <input 
                                    type="text" 
                                    className="w-full pl-10 pr-4 py-3 border-2 border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-900 rounded-xl focus:border-orange-400 focus:ring-4 focus:ring-orange-100 dark:focus:ring-orange-900/40 outline-none text-slate-800 dark:text-white font-medium transition-all"
                        ...
                        <div className="flex space-x-3">
                            ...
                            <button 
                                ...
                                className="flex-1 py-3 px-4 bg-orange-500 text-white rounded-xl font-bold hover:bg-orange-600 disabled:opacity-50 shadow-lg shadow-orange-200 dark:shadow-none transition-all"
                            >
                                Simpan
                            </button>
                        </div>
